...
export interface CreateMenuCommand {
  ...
  menuItems: CreateMenuItemCommand[];
 ...
 */
export class CreateMenuRequest extends ABaseHttpRequest<ApiResponse<void>> {
  private command: CreateMenuCommand;

  constructor(url: string, command: CreateMenuCommand) {
    super(url);
   ...
   */
  public static async send(command: CreateMenuCommand): Promise<ApiResponse<void>> {
    // Backend'deki ProductBox oluşturma endpoint'i için URL alınır.
    const url: string = ApiUrls.GetCreateMenuUrl();
    const request = new CreateMenuRequest(url, command);
