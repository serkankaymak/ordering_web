...
interface OrderItemProductComponentProps {
  menu: ProductModel;
}

const OrderItemProductComponent: IComponent<OrderItemProductComponentProps> = ({ menu }) => {
              ...
              <TableCell padding="none" colSpan={3} sx={{ pl: 1 }}>
                <Collapse className="w-100" in={isShowingSubItems}>
                  {menu.products.map((item) => (
                    <OrderItemProductComponent key={item.id} menu={item} />
                  ))}
                </Collapse>
              </TableCell>
            </TableRow>
...
export default React.memo(OrderItemProductComponent);
